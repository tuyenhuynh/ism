...
for($cnt = $startCnt; $cnt < $endCount; $cnt++)
{
	...
	if(($thisRow['doy'] >= $chartStartDate) && (dayofyear2date($thisRow['doy'],"Y-m-d",$row_FieldInfo['year']) <= date("Y-m-d")) )
	{
		...
		if($thisRow['rootDepth'] == 0)
		{
			$rz[$tCnt] = ""; //$sumIrrig;
		}
		else
		{
			$rz[$tCnt] = $thisRow['rootDepth'];
		}
...
$graph->title->Set("Crop Coefficient Chart");
$graph->img->SetMargin (45,40,0,65 );
...
$graph->yaxis->title->Set('Crop Coefficient');
$graph->y2axis->title->Set('Root Zone (in)');
...
$s2->SetWeight(1); 
$s2->SetColor("black");
//$s2->SetLegend("Crop Coefficient");
//$s2->SetFillFromYMin();
$s2->SetStyle('solid');
$s2->SetFillColor("blue@0.9");
unset($s2);
...
$graph->yaxis->scale->SetAutoMax(1.1);
$graph->yaxis->scale->SetAutoMin(0);
$graph->y2axis->scale->SetAutoMax($rz[count($rz)-1]*1.1);
